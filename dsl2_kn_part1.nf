...
process bwa_align {
	...
	bwa mem -t 12 -M -R "@RG\\tID:${pair_id}\\tLB:${pair_id}\\tPL:illumina\\tSM:${pair_id}\\tPU:${pair_id}" $ref ${paired_reads} > ${pair_id}.sam
...
process insert_summary {
	...
	script:
	"""
	cat $insert2_collection > InsertSizes_Final.txt
	"""
}
...
process run_report {
	...
	input:
	tuple val(pair_id), path('Bam_stat_summary_pf_final.tsv'), path('Bam_stat_summary_hs_final.tsv')
