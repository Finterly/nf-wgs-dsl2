...
# Knitr Options
knitr::opts_knit$set( #opts_chunk
  echo = FALSE,
	message = FALSE,
	warning = FALSE,
  fig.width = 9,
...
pf_stats <-read_tsv(file.path(params$directory, "Bam_stats_pf_Final.tsv"), show_col_types = FALSE)
hs_stats <-read_tsv(file.path(params$directory, "Bam_stats_hs_Final.tsv"), show_col_types = FALSE)
...
write.table(ratio_table, file='Ratios_hs_pf_reads.tsv', sep='\t', quote=FALSE, row.names=FALSE)
```
...
pf_stats %>%
  ...
  mutate(species = case_when(str_detect(species, "hs") ~ "Human", str_detect(species, "pf") ~ "P.falciparum")) -> reads_long
...
reads_long %>%
  ...
  ggplot(aes(x = log10(reads_mapped), y = pf_ratio, color = sample_id)) +
  geom_point() + 
...
insert_files <- list.files(path = params$directory, pattern = "insert.txt", recursive = TRUE, full.names = TRUE)
